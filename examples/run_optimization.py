...
def run_testcase2(gamble_size: int, k_size: int, bound: int, verbose: bool):
    Omega = list(range(gamble_size))
    K = random_gambles(k_size, gamble_size, bound)
    p = generate_mass_function(Omega)
    assert is_mass_function(p)
    ...
    for epsilon in [0.000001, 0.00001, 0.0001, 0.001, 0.1]:
        ...
        print(f'epsilon = {epsilon}\nP = {print_lower_prevision_function(P)}')
        watch = StopWatch()
        result = incurs_sure_loss(P_p, Omega, verbose)
...
def main():
    ...
    for _ in range(args.count):
        if args.test == 1:
            ...
            run_testcase2(args.gamble_size, args.k_size, args.coordinate_bound, args.verbose)
        else:
