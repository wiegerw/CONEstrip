...
from z3 import *
from conestrip.cones import Gamble, ConvexCombination, linear_combination, gambles_to_polyhedron, ConeGenerator, \
    GeneralCone
...
def random_inside_point(R: ExtendedConeGenerator) -> Tuple[Gamble, ConvexCombination]:
    """
...
def random_border_point(R: ExtendedConeGenerator) -> Tuple[Gamble, ConvexCombination]:
    """
...
def add_random_border_cone(R: ExtendedConeGenerator) -> ExtendedConeGenerator:
    # converts indices to points
    def make_facet(indices: Tuple[int]) -> ExtendedConeGenerator:
        return ExtendedConeGenerator([R.vertices[i] for i in indices])
...
def add_random_border_cones(R: ExtendedGeneralCone, n: int, allow_multiple_children: bool = False) -> None:
    """
...
def random_extended_cone_generator(dimension: int, generator_size: int, bound: int, normal=None) -> ExtendedConeGenerator:
    return ExtendedConeGenerator(random_cone_generator(dimension, generator_size, bound, normal))


def random_extended_general_cone(cone_size: int, dimension: int, generator_size: int, bound: int) -> ExtendedGeneralCone:
    return ExtendedGeneralCone([random_extended_cone_generator(dimension, generator_size, bound) for _ in range(cone_size)])
